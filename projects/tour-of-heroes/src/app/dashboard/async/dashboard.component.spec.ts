...
import {predefinedHttpCallInstructionsAsync, runTasksUntilStableAsync, HttpCallInstructionAsync} from 'ngx-testbox/testing';
...
describe('DashboardComponent', () => {
  ...
  it('should display "Top Heroes" in the title', async () => {
    await initComponent();
    expect(harness.elements.title.getTextContent()).toBe('Top Heroes');
  });

  describe('heroes menu', () => {
    it('should show heroes when server responds with heroes', async () => {
    ...
    it('should display hero name for each hero', async () => {
      await initComponent([getHeroesSuccessHttpCallInstruction(5)]);
